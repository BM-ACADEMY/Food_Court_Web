...
exports.createTransaction = async (req, res) => {
  try {
    ...
    console.log(
      "createTransaction - senderRoleId:",
      senderRoleId,
      "receiverRoleId:",
      receiverRoleId
    );
    ...
    console.log(
      `Transaction created: ${transaction._id} - ${transaction.transaction_id}`
    );
...
exports.processPayment = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      message: `Payment successful! New customer balance: ₹${(
    ...
    const enrichedTransactions = await Promise.all(
      transactions.map(async (transaction) => {
        ...
        if (customerUserId) {
          ...
          console.log(
            `Transaction ${transaction.transaction_id}: No customer involved`
          );
          customer_id = "N/A";
        }
  ...
  try {
    ...
    try {
      transactions = await Transaction.find(filter)
        ...
        .lean();
      console.log(
        `Fetched ${transactions.length} transactions for userId: ${userId}`
      );
    } catch (dbError) {
      ...
      transactions.map(async (txn) => {
        ...
        try {
          if (user._id) {
            ...
            console.warn(
              `No valid user._id for transaction ${txn.transaction_id || "N/A"}`
            );
            customer_id = `CUST${txn.transaction_id?.slice(-3) || "000"}`;
          }
        } catch (customerErr) {
          console.warn(
            `Failed to fetch customer_id for user ${
              user._id || "unknown"
            } in transaction ${txn.transaction_id || "N/A"}:`,
            customerErr.message
          );
          customer_id = `CUST${txn.transaction_id?.slice(-3) || "000"}`;
        }
    ...
    console.log(
      `Exported ${formattedTransactions.length} transactions for userId: ${userId}`
    );
    return res
      .status(200)
    ...
    console.error(
      "Error in exportTransactionHistoryByUserId:",
      error.message,
      error.stack
    );
    return res.status(500).json({
