    ...
    if (open) {
      ...
      Promise.all([fetchRoles, fetchLocations])
        .then(([rolesRes, locationRes]) => {
          ...
          // Filter roles to only include role-3 (Treasury-Subcom) and role-4 (Restaurant)
          const filteredRoles = rolesFromApi.filter(
            (role) => role.role_id === "role-3" || role.role_id === "role-4"
          );

          setRoles(filteredRoles);
          setLocations(locationsFromApi);

          const defaultRoleId = defaultValues?.role_id || filteredRoles[0]?._id || "";
