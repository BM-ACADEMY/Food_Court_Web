...
//   const transactionTypes = ["All", "Transfer", "Refund"];
...
//                   <TableCell
//                     className={`p-4 font-semibold ${transaction.type === "Refund" ? "text-red-600" : "text-green-600"
//                       }`}
//                   >
...
//               className={`transition-colors duration-200 ${page === 1 || isLoading
...
//                   }}
//                   disabled={isLoading}
//                   className={`px-3 py-1 rounded-md transition-colors duration-200 ${page === pageNum
//                       ? "bg-black text-white"
//                       : "text-gray-800 hover:bg-gray-100"
//                     }`}
//                 >
...
//               className={`transition-colors duration-200 ${page === totalPages || isLoading
//                   ? "text-gray-400 cursor-not-allowed"
//                   : "text-gray-800 hover:bg-gray-100"
//                 }`}
//             />
...
const RestaurantHistory = () => {
  ...
  const [date, setDate] = useState(new Date());
  const [isCalendarOpen, setIsCalendarOpen] = useState(false);
  const [transactionType, setTransactionType] = useState("All"); // Default to "All"
  ...
  // Update sortConfig when sortValue changes
  const [sortValue, setSortValue] = useState("customer-asc");
  useEffect(() => {
    if (sortValue) {
      const [key, direction] = sortValue.split("-");
      setSortConfig({ key, direction });
    ...
    debounce(async (params) => {
      try {
        ...
        const res = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/transactions/fetch-treasury-subcom-restaurant-history`,
          {
            params,
            withCredentials: true,
          }
  ...
  useEffect(() => {
    if (!loading && user) {
      const queryParams = {
        transactionType: transactionType === "All" ? undefined : transactionType, // Send undefined for "All" to fetch all types
        fromDate: format(startOfDay(date), "yyyy-MM-dd"),
        toDate: format(endOfDay(date), "yyyy-MM-dd"),
        search: searchQuery,
        page,
  ...
  const handleSortChange = (key) => {
    ...
    setSortValue(`${key}-${sortConfig.key === key && sortConfig.direction === "asc" ? "desc" : "asc"}`);
  };
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 md:p-8 max-w-7xl mx-auto bg-gray-50">
      {/* Title */}
      <Card className="mb-6">
        <CardContent className="p-4">
          <CardTitle className="text-xl sm:text-2xl font-bold" style={{ color: "#070149" }}>
      ...
      <div className="flex flex-col sm:flex-row sm:flex-wrap items-start sm:items-center justify-between gap-4 mb-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 w-full sm:w-auto">
          <Select value={selectedRestaurant} onValueChange={setSelectedRestaurant} disabled={isLoading}>
            <SelectTrigger className="w-full sm:w-40 border-gray-300 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200 text-gray-700">
              <SelectValue placeholder="Select Restaurant" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">All Restaurants</SelectItem>
          ...
          <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
            <PopoverTrigger asChild>
              <Button
                variant="outline"
                className="w-full sm:w-40 justify-between text-sm border-gray-300 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200 text-gray-700"
                disabled={isLoading}
              >
                {format(date, "PPP")}
                <CalendarDays className="ml-2 h-4 w-4 text-gray-500" />
        ...
        <Button
          onClick={() => setOpenDialog(true)}
          className="w-full sm:w-auto bg-[#2d2d71] text-white hover:bg-[#44447c] gap-2 transition-colors duration-200"
          disabled={isLoading || isExporting}
        >
          <Download size={16} /> Export
        </Button>
        ...
        <PaginationContent>
          <PaginationItem>
            <PaginationPrevious
              ...
              disabled={page === 1 || isLoading}
              className={`transition-colors duration-200 ${page === 1 || isLoading ? "text-gray-400 cursor-not-allowed" : "text-gray-800 hover:bg-gray-100"}`}
            />
          </PaginationItem>
          {[...Array(Math.min(totalPages, 3))].map((_, i) => {
            const pageNum = i + 1;
            return (
              <PaginationItem key={pageNum}>
                <PaginationLink
                  ...
                  disabled={isLoading}
                  className={`px-3 py-1 rounded-md transition-colors duration-200 ${page === pageNum ? "bg-black text-white" : "text-gray-800 hover:bg-gray-100"}`}
                >
                  {pageNum}
                </PaginationLink>
              </PaginationItem>
          ...
          <PaginationItem>
            <PaginationNext
              ...
              disabled={page === totalPages || isLoading}
              className={`transition-colors duration-200 ${page === totalPages || isLoading ? "text-gray-400 cursor-not-allowed" : "text-gray-800 hover:bg-gray-100"}`}
            />
          </PaginationItem>
        </PaginationContent>
      </Pagination>
